...
public class Util {
	...
	public static final void log(Object msg) {
		Bukkit.getServer().getConsoleSender().sendMessage("[" + FS.getPlugin().getName() + "] " + color("" + msg));
	}
	...
	public static final Player uuidToPlayer(UUID uuid) {
		return FS.getPlugin().getServer().getPlayer(uuid);
	}
	
	public static final String getCFString(String s) {
		return color(FS.getPlugin().getConfig().getString(s));
	}
	
	public static final int getCFInt(String s) {
		return FS.getPlugin().getConfig().getInt(s);
	}
	...
	public static final String format(double i) {
		String better = FS.getEcon().format(i).replaceAll("[^\\d.]", "");
